...
Ax.v = /*START*/
{"build":56,"release":"0.2","dev":true,"date":1211755239.69}
/*STOP*/
...
toolboxicons: "../img/icon/"
 ...
 /*awesome little grey boxes that fall from the sky... stolen from Ext dos */
...
var ErrorWindow = new Ext.Window({
	...
	
	{
		...
		handler: function(){
			msg("Sending Bug Report","We are sending your bug report");
			ErrorWindow.close();
		}
	}
...
{text: "Debug Console", iconCls: "tb_debug", handler: Ext.log},
...
{text: "Key Shortcuts", iconCls: "tb_keyboard", handler: Ax.keyGuide},
{text: "Manual", iconCls: "tb_docs", handler: Ax.loadManual},
{text: "FAQ", iconCls: "tb_docs", handler: Ax.loadFAQ},
...
//clicksToEdit:1,
ds:/*BEGIN*/new Ext.data.Store({reader: new Ext.data.ArrayReader({}, [{name: 'comment'}]),data: [['Layer 1']]})/*END*/,
cm:/*BEGIN*/new Ext.grid.ColumnModel([{
header: 'Comment',
 dataIndex: 'comment',
...
Ax.addLayer = function(){
  var layer = document.createElement("tr");

  Ax.layers["Layer "+(Ax.layers.length+1).toString()]=layer
  Ax.current.layer = layer
  Ax.timeline.appendChild(layer)
  return layer;
}
Ax.addFrame = function(frame,layer){
  ...
  Ax.current.layer.appendChild(frame_cell)
}
//#99BBE8
